...
describe("Shell Completion Backends", function()
  ...
  describe("Script Generation (Thin Shims)", function()
    ...
    it("generates valid Bash completion script", function()
      ...
      assert.truthy(script:find("__stellar_complete%(%)"), "defines bash completion function")
      assert.truthy(script:find("%-%-__clingy%-complete bash"), "invokes hidden entrypoint with bash")
      assert.truthy(script:find("complete %-o default %-F __stellar_complete stellar"), "registers complete command")
    end)
    ...
    it("generates valid Zsh completion script", function()
      ...
      assert.truthy(script:find("_describe %-t commands 'stellar' candidates"), "uses _describe for tags")
    end)

    it("generates valid Fish completion script", function()
      local script = comp_mod.completion_script(app, "fish")
      assert.truthy(script:find("function __stellar_complete"), "defines fish function")
      assert.truthy(script:find("%-%-__clingy%-complete fish"), "invokes hidden entrypoint with fish")
      assert.truthy(script:find("complete %-c stellar %-f %-a '%(__stellar_complete%)'"), "registers fish completer")
    end)
  ...
  describe("Response Rendering: Bash Backend", function()
    ...
    it("emits directive header when directives are present", function()
      ...
      local out = backends.bash.render(resp)
      assert.truthy(out:find("^:directive:filenames,nospace"), "emits directive prefix")
      assert.truthy(out:find("main.lua"))
    end)
  ...
  describe("Response Rendering: Zsh Backend", function()
    it("renders value:description pairs with escaped colons", function()
      local resp = response.create()
      resp:add("port", "Port: 8080 or 443")
      resp:add("raw")

      local out = backends.zsh.render(resp)
      assert.truthy(out:find("port:Port\\: 8080 or 443"), "escapes colon in description")
      assert.truthy(out:find("\nraw$") or out:find("^raw") or out:find("\nraw\n"))
    end)
    ...
    it("emits directive header for filenames and directories in zsh", function()
      ...
      local out = backends.zsh.render(resp)
      assert.truthy(out:find("^:directive:dirnames"))
    end)
  end)
  ...
  describe("Response Rendering: Fish Backend", function()
    it("renders tab-separated value and description", function()
      ...
      assert.truthy(out:find("start\tStart the daemon"), "separates value and description by tab")
      assert.truthy(out:find("stop"), "includes candidate without description")
      assert.is_nil(out:find("stop\t"), "does not append tab if description is nil")
    end)
  end)
  ...
  describe("Response Rendering: PowerShell Backend", function()
    it("renders tab-separated value and description for PowerShell parsing", function()
      ...
      assert.equal(out, "init\tInitialize workspace")
    end)
  end)
